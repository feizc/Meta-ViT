...
class WeightformerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__() 
        # 1. weight dict emb, relative position emb, model id emb 
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.model_type_embeddings = nn.Embedding(config.type_model_size, config.hidden_size)
    ...
    def forward(
        self, input_ids=None, model_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0
    ):
        ...
        if model_type_ids is None:
            if hasattr(self, "model_type_ids"):
                buffered_token_type_ids = self.model_type_ids[:, :seq_length]
                buffered_token_type_ids_expanded = buffered_token_type_ids.expand(input_shape[0], seq_length)
                model_type_ids = buffered_token_type_ids_expanded
            else:
                model_type_ids = torch.zeros(input_shape, dtype=torch.long, device=self.position_ids.device)

        inputs_embeds = self.word_embeddings(input_ids)
        token_type_embeddings = self.model_type_embeddings(model_type_ids)
...
class WeightformerLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = WeightformerAttention(config, position_embedding_type="absolute")
